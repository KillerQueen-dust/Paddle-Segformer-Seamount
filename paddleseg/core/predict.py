...
def preprocess(im_path, transforms):
    ...
    data['img'] = paddle.to_tensor(data['img'], dtype='bfloat16')
    return data
...
def predict(model,
    ...
    Args:
        ...
        additional_added (str, optional): Additional directory of the images to be added. Default: None.
    ...
    # personal
    npy_saved_dir = os.path.join(save_dir, 'logits')
    ...
    with paddle.no_grad():
        for i, im_path in enumerate(img_lists[local_rank]):
            ...
            # personal: save predicted npy files
            npy_saved_path = os.path.join(
                npy_saved_dir, os.path.splitext(im_file)[0] + ".npy")
            mkdir(npy_saved_path)
            np.save(npy_saved_path, pred)
