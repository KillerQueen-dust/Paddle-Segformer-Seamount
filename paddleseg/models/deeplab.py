...
__all__ = ['DeepLabV3P', 'DeepLabV3', 'DeepLabV2']
...
class DeepLabV2(nn.Layer):
    ...
    def forward(self, x):
        feat_list = self.backbone(x)
        logit_list = self.head(feat_list[3])
        return [F.interpolate(
            logit, paddle.shape(x)[2:], mode='bilinear')
...
class DeepLabV2Head(nn.Layer):
    def __init__(self,
        ...
        self.aspp = ASPPModule_Deeplabv2(
            aspp_ratios=aspp_ratios,
            in_channels=backbone_channels,
            out_channels=num_classes)

    def forward(self, feat_map):
        logit_list = [self.aspp(feat_map)]
        return logit_list


class ASPPModule_Deeplabv2(nn.Layer):
    """
